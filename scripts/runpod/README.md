...
- **flashinfer's sampler JIT can fail when the pod's CUDA *toolkit* (nvcc) doesn't match the cu13 torch wheels** (e.g. the `runpod-torch-v21` template ships nvcc 11.8 while vLLM 0.23 pulls torch 2.11+cu130). Symptom: `EngineCore failed to start` with a `subprocess.CalledProcessError` from `ninja -C .../flashinfer/.../sampling` during init (right after a `Using FlashInfer for top-p & top-k sampling` log line). Workaround: **export `VLLM_USE_FLASHINFER_SAMPLER=0`** before running — it falls back to vLLM's native torch sampler (negligible cost, especially at `temperature 0` where sampling is just argmax) and skips the JIT compile. This is deliberately **not hardcoded** in the scripts because it depends on the pod's toolkit/driver combo: a pod whose nvcc matches the torch CUDA build won't hit it. Prefer a template whose CUDA toolkit matches the torch wheels; otherwise set the env var per run (observed 2026-06-16 on an A40 + `runpod-torch-v21`).
