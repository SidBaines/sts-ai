...
- `run_sweep.py` — batched multi-model sweep: models × {thinking on/off} × seeds via the cross-rollout orchestrator (`sts_ai.parallel_rollout`, `--batch-size` K). Writes per-rollout JSONL + `.meta.json` under `<output-dir>/<agent_label>/`. One model load is reused across both thinking modes; a model that fails to load is skipped. This is the throughput path (~K× via `mlx_lm.batch_generate`); `run_rollout.py`/`run_batch.py` remain the serial path. **Measured per-model rollout speeds (for planning how long a run takes) are in [`docs/throughput_benchmarks.md`](../docs/throughput_benchmarks.md).**
