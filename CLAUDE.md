...
- **The Python↔C++ binding is maintained as a patch**, `patches/sts_lightspeed_python_api.patch`, applied to the upstream clone by the build script. To change the binding, **edit the patch and rebuild** — do not edit `external/sts_lightspeed/` directly, as that clone is untracked and your changes won't be versioned.
