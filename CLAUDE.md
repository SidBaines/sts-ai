...
- **Test before claiming done.** Run `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -t .` and at least one smoke rollout — use a **bounded** one, `scripts/run_rollout.py --agent random --seed 3 --max-act 1 --battle-simulations 50` (seconds) — before reporting a change as working. Do **not** use bare `--agent random`: at its defaults (seed 1, 2,000 battle simulations, 3 acts, random pathing into many fights) it spins at 100% CPU with no output for over an hour, so it is useless as a smoke test and indistinguishable from the known simulator hang. Add a regression test for every bug you fix. Tests are tiered: `tests/unit` (pure Python, no build) and `tests/integration` (needs the built simulator, gated by `@requires_simulator`); see [`tests/CLAUDE.md`](tests/CLAUDE.md). Note the `-t .` flag — it's required now that the tiers are packages.
...
- [`docs/ooc_rl_v1_status.md`](docs/ooc_rl_v1_status.md) — status, findings and resume instructions for the paused `ooc_grpo_v1` out-of-combat RL run.
...
- **The upstream simulator has uninitialized-memory UB** (default-constructed `GameContext`/`BattleContext` had uninitialized `potions`). Symptoms are build-/layout-dependent (a given seed may crash on one build and hang on another), which breaks naive cross-build reproducibility. If you see a seed behave differently after an unrelated rebuild, suspect UB, not your change. The hang variant is also **state-dependent, so seed screening does not transfer across policies**: a cohort screened clean under one agent can still wedge under another whose decisions reach different combat states (bitten 2026-08-17: ooc_grpo_v1 iter-4 wedged on a screened seed; in-process rollout runners freeze whole-loop). Long unattended runs need a stall watchdog that kills and resumes with a bumped `run_grpo.py --policy-seed-salt` so the redone iteration re-rolls instead of deterministically replaying into the same state. A second open bug in the same family produces impossible "phantom" enemy/player powers in saved combat states; affected windows are quarantined in `configs/competence/state_sanity_quarantine_v1.json` (detector: `scripts/audit_state_sanity.py`). Detailed write-ups live in git history (`docs/simulator_issue_handoff.md`).
