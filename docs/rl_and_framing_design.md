...
1. **Invalid outputs terminate cleanly.** `parse_json_action` still returns `action_index=0, valid=False` as a structured placeholder, but rollout generation records the invalid response and stops with `agent_invalid` before executing a fallback action. Naively training on invalid terminal records should still be masked or assigned a format penalty, but malformed generations no longer ride a fallback trajectory reward.
   - ⚠ **Truncation conflates format failure with policy quality — report `agent_invalid` rate *alongside* every outcome metric.** Because a single unrecoverable invalid decision ends the whole episode, rollout length/outcome now depends partly on format-compliance, not just decision quality: a truncated episode never reaches `VICTORY`, so a format failure scores as a loss. Two consequences: (a) the **thinking-vs-no-thinking** comparison can misattribute a gap — a thinking arm that is marginally worse at emitting the closing JSON (the plan notes ~6% truncate even at 4096) will look much worse on floor/win-rate for format reasons, not policy reasons — so always read outcome next to the `agent_invalid`/`unexecuted` rate (now surfaced by `summarize_rollouts.py` and `audit_rollouts.py`); (b) for the temp>0 on-policy RL arm, invalid rate rises and truncation becomes common, so the episode reward mixes format and policy signal — mask the terminal invalid record or apply an explicit format penalty rather than scoring it as a plain loss (see also gotcha 5).
